...
DIR_COW200 = os.path.join(DIR_DATA, "1b_angle_s2t", "tv")
...
def main(args):
    ...
    # need to change for each dataset --------------------
    folder_name_string = suffix+"_s2t"+f"_{yolo_base}_{n_train}_{i}"
    ...
    # log
    print("----------------------------------------------------------------------------")
    print(f"n_train: {n_train}, s2t_yolo_base: {yolo_base}, i: {i}, {suffix}")
    print("----------------------------------------------------------------------------")
    ...
    # need to change for each dataset ------------------------
    name_task = f"n{n_train}_{yolo_base}_i{i}"+"_s2t"
    ...
    checkpoint_dir = ROOT + '/checkpoints/' + 'n' + \
        str(n_train) + '_' + yolo_base + '_' + 'i' + str(i) + '_s2t' #need to change ----------------
    ...
    yolo_nas.evaluate_test_set(
        ROOT, yolo_base, '1b_angle_s2t', 'exp', n_train, i) #need to change ---------------------------
