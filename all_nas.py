...
DIR_COW200 = os.path.join(DIR_DATA, "4_all", "tv")
...
def main(args):
    ...
    # need to change for each dataset --------------------
    folder_name_string = suffix+"_all"+f"_{yolo_base}_{n_train}_{i}"
    ...
    # log
    print("----------------------------------------------------------------------------")
    print(f"n_train: {n_train}, all_yolo_base: {yolo_base}, i: {i}, {suffix}")
    print("----------------------------------------------------------------------------")
    ...
    # need to change for each dataset ------------------------
    name_task = f"n{n_train}_{yolo_base}_i{i}"+"_all"
    ...
    checkpoint_dir = ROOT + '/checkpoints/' + 'n' + \
        str(n_train) + '_' + yolo_base + '_' + 'i' + str(i) + '_all' #need to change -----------------
    ...
    yolo_nas.evaluate_test_set(
        ROOT, yolo_base, '4_all', 'exp', n_train, i) #need to change ------------------
