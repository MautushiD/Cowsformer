...
DIR_COW200 = os.path.join(DIR_DATA, "3_breed", "tv")
...
def main(args):
    ...
    # need to change for each dataset --------------------
    folder_name_string = suffix+"_breed"+f"_{yolo_base}_{n_train}_{i}"
    ...
    # need to change for each dataset ------------------------
    name_task = f"n{n_train}_{yolo_base}_i{i}"+"_breed"
    ...
    yolo_nas.evaluate_test_set(
        ROOT, yolo_base, '3_breed', 'exp', n_train, i)  # need to change ---------------------------
